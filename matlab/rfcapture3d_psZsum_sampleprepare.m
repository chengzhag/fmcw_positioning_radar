...
sFileTime='../data/timeStr_200kHz_2000rps_4rpf_4t12r_stand_fall.txt';
load(sFileData)
...
%% 选择要切割的样本标签
lbSam=input('输入要切割的样本标签：');
isSam=find(lbsChange(:,1)==lbSam);

%% 切割样本
isSamW=-round(tSam*fF/2):round(tSam*fF/2);
tsSamW=isSamW/fF;
psZsumSam=zeros(size(psZsum,1),length(isSamW),length(isSam));

for i=1:length(isSam)
    tSamCen=lbsChange(isSam(i),2);
...
if doShowSam
    ...
    for i=1:size(psZsumSam,3)
        ...
        imagesc(tsSamW+lbsChange(isSam(i),2),zsF,psZsumSamAj);
        set(gca, 'XDir','normal', 'YDir','normal');
        title(['样本' num2str(i) ' 共' num2str(size(psZsumSam,3)) '个样本']);
        xlabel('t(s)');
        ylabel('z(m)');
        pause(2);
    end
