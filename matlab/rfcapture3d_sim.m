...
xsV=reshape(xss,1,numel(xss));
ysV=reshape(yss,1,numel(yss));
zsV=reshape(zss,1,numel(zss));
...
rsTsZRT=zeros(1,length(zsV),nRx,nTx,'single');%r(n,m)(X(ts),Y(ts),z)，（ts为长时间）
for iRx=1:nRx
    for iTx=1:nTx
        rsTsZRT(:,:,iRx,iTx)=sqrt( ...
            (xsV-repmat(antCoor(iRx,1),1,length(zsV))).^2 ...
            + (ysV-repmat(antCoor(iRx,2),1,length(zsV))).^2 ...
            + (zsV-repmat(antCoor(iRx,3),1,length(zsV))).^2 ...
            ) ...
            + sqrt( ...
            (xsV-repmat(antCoor(iTx+nRx,1),1,length(zsV))).^2 ...
            + (ysV-repmat(antCoor(iTx+nRx,2),1,length(zsV))).^2 ...
            + (zsV-repmat(antCoor(iTx+nRx,3),1,length(zsV))).^2 ...
            );
    end
end
...
rsRampRTZ=repmat(permute(rsTsZRTGPU(1,:,:,:),[1,3,4,2]),length(tsRamp),1);
fTsrampRTZ=exp( ...
    1i*2*pi*fBw*fTr.*rsRampRTZ/3e8 ...
    .*repmat(tsRampGPU',1,size(rsRampRTZ,2),size(rsRampRTZ,3),size(rsRampRTZ,4)) ...
    ) ...
    .*exp( ...
    1i*2*pi*rsRampRTZ/dLambda ...
    );
